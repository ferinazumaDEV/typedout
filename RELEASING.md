...
How a release of `typedout-py` is made, what evidence it leaves behind, how anyone
...
gh release download vX.Y.Z --repo ferinazumaDEV/typedout
...
```bash
gh run download <run-id> --repo ferinazumaDEV/typedout --name dist --dir dist
gh release create vX.Y.Z --repo ferinazumaDEV/typedout --title vX.Y.Z \
